...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const [{ data: actividad }, { data: inscripciones }, { data: invitados }] = await Promise.all([
    admin.from('actividades').select('titulo, fecha_inicio, precio, precio_invitado').eq('id', actividadId).single(),
    ...
    admin.from('actividades_invitados')
      .select('nombre, email, tipo, estado, fecha_inscripcion, fecha_pago, precio, notas, socios(nombre, apellidos)')
      .eq('actividad_id', actividadId)
      .order('fecha_inscripcion'),
  ])
  ...
  const precioSocio = actividad?.precio ?? null
  const precioInv = actividad?.precio_invitado ?? actividad?.precio ?? null
  ...
  for (const inv of (invitados ?? [])) {
    ...
    const tipoLabel = inv.tipo === 'invitado' ? 'Invitado' : 'Acompañante'
    const precio = inv.precio ?? (inv.tipo === 'invitado' ? precioInv : precioInv)
    rows.push(csvRow([tipoLabel, null, null, inv.nombre, null, inv.email, inv.estado, inv.fecha_inscripcion?.slice(0, 10), inv.fecha_pago?.slice(0, 10), precio, añadidoPor, inv.notas]))
  }
